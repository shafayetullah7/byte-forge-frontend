...
export const route = {
  preload: () => Promise.all([getSellerCampaigns({ page: 1, limit: 20 }), getShop()]),
...
export default function SellerCampaignsPage() {
  ...
  const statusOptions = createMemo(() =>
    STATUS_FILTERS.map((value) => ({
      value,
      label: value ? moderationLabel(t, value) : t("seller.campaigns.filterAll"),
    })),
  );
  ...
  const handleLifecycleConfirm = async () => {
    ...
    if (result?.success) {
      toaster.success(
        pending.type === "archive"
          ? t("seller.campaigns.archived")
          : t("seller.campaigns.deleted"),
      );
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="max-w-xs">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          {t("seller.campaigns.filterStatus")}
        </label>
        <Select
                      ...
                      <tr class="border-t border-cream-200 dark:border-forest-700">
                        ...
                        <td class="px-4 py-3 text-right space-x-3">
                          ...
                          <Show when={item.moderationStatus === "APPROVED" && shop()?.slug}>
                            <A
                              href={`/shops/${shop()!.slug}/campaigns/${item.slug}`}
                              class="text-forest-600 hover:underline"
                              target="_blank"
