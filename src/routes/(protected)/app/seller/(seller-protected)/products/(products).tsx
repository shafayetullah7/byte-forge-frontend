import { createSignal, createMemo, For, Show, Suspense, ErrorBoundary, createEffect } from "solid-js";
...
// Tag groups (matches backend PublicTagGroupResponse structure)
const TAG_GROUPS = [
  {
    id: "group-1",
    slug: "size",
...
// Hierarchical categories (matches backend PublicCategoryResponse)
...
export default function ProductsPage() {
  ...
  createEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (!target.closest("[data-tag-dropdown]")) {
        // Don't auto-close to avoid interfering with selection
      }
    };
  ...
  const filteredProducts = createMemo(() => {
    ...
    // Tag filter (products must have ALL selected tags)
  ...
  return (
    <SafeErrorBoundary
      fallback={(err, reset) => (
        <InlineErrorFallback error={err} reset={reset} label="products" />
      )}
    >
      <div class="min-h-screen py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="bg-white dark:bg-forest-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6">
            <div class="p-4">
              {/* Main Row: Search + Quick Filters + Sort */}
          ...
          <ErrorBoundary
            fallback={(error) => (
              <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
                <p class="text-red-700 dark:text-red-300">Failed to load products: {error.toString()}</p>
              </div>
            )}
          >
            <Show
                ...
                <div class="bg-white dark:bg-forest-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
                  ...
                  <p class="text-gray-500 dark:text-gray-400 mb-6">
                    {hasActiveFilters()
                      ...
                      : "Start by adding your first plant product"}
                  </p>
                  <Show
