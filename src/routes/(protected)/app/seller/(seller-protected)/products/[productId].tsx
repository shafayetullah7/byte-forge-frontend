import { ErrorBoundary, Suspense, For, createMemo, Show } from "solid-js";
...
import { getProductTranslation, type ProductStatus, type ProductType } from "~/lib/api/types/seller.types";
...
export default function ProductDetailLayout(props: RouteSectionProps) {
  ...
  const productType = createMemo(() => product()?.productType ?? "");
  const productStatus = createMemo(() => product()?.status ?? "");
  const productId = createMemo(() => product()?.id ?? "");

  const typeColors = createMemo(() => {
    const type = productType();
    if (!type) return { bg: "", text: "", border: "" };
    return getProductTypeColor(type as ProductType);
  });
                ...
                <div class="flex items-start gap-4">
                  ...
                  <div>
                    <div class="flex items-center gap-3 flex-wrap mb-2">
                      ...
                      <Badge variant={getStatusVariant(productStatus() as ProductStatus)}>
                        {getStatusLabel(productStatus() as ProductStatus)}
                      </Badge>
                      <span class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${typeColors().bg} ${typeColors().text} ${typeColors().border}`}>
                        {getProductTypeLabel(productType() as ProductType)}
                      </span>
                    </div>
