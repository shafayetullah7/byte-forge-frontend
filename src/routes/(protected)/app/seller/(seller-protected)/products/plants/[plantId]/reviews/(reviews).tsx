      ...
      <div class="space-y-6">
        <SectionCard title="Rating Summary">
          <div class="text-center mb-4">
            ...
            <div class="flex justify-center mt-2"><StarRatingDisplay rating={MOCK_REVIEWS_SUMMARY.average} size="w-5 h-5" /></div>
              ...
              {(dist) => (
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 h-2.5 bg-cream-100 dark:bg-forest-700 rounded-full overflow-hidden">
                    <div class="h-full bg-cream-500 rounded-full transition-all" style={{ width: `${dist.percentage}%` }} />
                  </div>
        ...
        <SectionCard title="Filter Reviews">
          <div class="space-y-2">
            <button onClick={() => setReviewFilter(null)} class={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${reviewFilter() === null ? "bg-forest-100 text-forest-800 dark:bg-forest-900/40 dark:text-forest-300 font-medium" : "text-gray-700 dark:text-gray-300 hover:bg-cream-50 dark:hover:bg-forest-700"}`}>
            ...
            <For each={[5, 4, 3, 2, 1]}>
              {(stars) => {
                ...
                return (
                  <button onClick={() => setReviewFilter(reviewFilter() === stars ? null : stars)} class={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${reviewFilter() === stars ? "bg-forest-100 text-forest-800 dark:bg-forest-900/40 dark:text-forest-300 font-medium" : "text-gray-700 dark:text-gray-300 hover:bg-cream-50 dark:hover:bg-forest-700"}`}>
                    <span class="flex items-center gap-1">{stars} <StarIcon class="w-3.5 h-3.5 text-cream-500" /></span>
            ...
            <For each={MOCK_REVIEWS_SUMMARY.highlights}>
              {(highlight) => (
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-xs font-medium text-forest-600 dark:text-forest-400 bg-forest-100 dark:bg-forest-900/40 px-2 py-0.5 rounded-full">{highlight.count}</span>
                </div>
              )}
            </For>
          </div>
            ...
            <ErrorBoundary fallback={(error) => <SectionErrorFallback error={error} title="review" />}>
              <div class="bg-white dark:bg-forest-800 rounded-xl border border-cream-200 dark:border-forest-700 shadow-sm p-5">
                <div class="flex items-start justify-between mb-3">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-forest-100 dark:bg-forest-700 flex items-center justify-center"><UserIcon class="w-5 h-5 text-forest-600 dark:text-forest-400" /></div>
                    <div>
                      <div class="flex items-center gap-2">
                        <p class="text-sm font-semibold text-forest-800 dark:text-cream-50">{review.customerName}</p>
                        {review.verifiedPurchase && <CheckBadgeIcon class="w-4 h-4 text-forest-600 dark:text-forest-400" />}
                      </div>
                ...
                {review.images.length > 0 && (
                  <div class="flex gap-2 mb-3">
                    <For each={review.images}>
                      {(img) => <div class="w-16 h-16 rounded-lg bg-cream-100 dark:bg-forest-700 border border-cream-200 dark:border-forest-600 flex items-center justify-center" />}
                    </For>
                  </div>
                )}
