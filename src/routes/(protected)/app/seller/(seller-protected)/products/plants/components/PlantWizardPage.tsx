import { createSignal, createMemo, Show, createEffect, ErrorBoundary, Suspense, For, onMount } from "solid-js";
import { createStore } from "solid-js/store";
import { A, createAsync, action, useAction, useSubmission, useNavigate, useSearchParams } from "@solidjs/router";
...
import { Step2CategoryTags } from "../new/Step2CategoryTags";
import { Step3Classification } from "../new/Step3Classification";
import { Step4Variants, type VariantStore } from "../new/Step4Variants";
import { Step5CareProfile } from "../new/Step5CareProfile";
...
const savePlantAction = action(async (data: SavePlantActionData) => {
  "use server";
  try {
    if (data.saveAsDraft) {
...
export function PlantWizardPage(props: PlantWizardPageProps) {
  ...
  // ---- Server Action ----
  const savePlantTrigger = useAction(savePlantAction);
  ...
  const thumbnailUpload = useImageUpload({
    ...
    onClear: () => setForm("thumbnail", { id: null, url: null }),
  });

  const effectiveThumbnailPreview = createMemo(
    () => thumbnailUpload.preview() ?? form.thumbnail.url,
  );
