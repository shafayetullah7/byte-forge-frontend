...
export function VariantCatalogFields(props: {
  ...
  const visibleVariants = createMemo(() => {
    if (!props.variantFilterId) return props.variants;
    return props.variants.filter((v) => v.id === props.variantFilterId);
  });
  ...
  return (
    <div class="space-y-6">
      ...
      {/* Variant Cards */}
      <Index each={visibleVariants()}>
        {(variant, visibleIndex) => {
          const i = props.variantFilterId
            ? props.variants.findIndex((v) => v.id === variant().id)
            : visibleIndex;
          const baseCount = props.variants.filter(v => v.isBase).length;
