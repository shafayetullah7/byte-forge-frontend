...
import type { ShopTranslationDto } from "~/lib/api/endpoints/seller/shop-detail.api";
...
export default function ShopInfoEditModal(props: ShopInfoEditModalProps) {
  ...
  const enTranslation = () => props.shop?.translations?.find((tr) => tr.locale === "en");
  const bnTranslation = () => props.shop?.translations?.find((tr) => tr.locale === "bn");

  const [, { Form, Field }] = createForm<ShopInfoFormData>({
    initialValues: {
      ...
      <div class="overflow-y-auto px-1 space-y-4 custom-scrollbar" style="max-height: 70vh;">
        <Form onSubmit={handleSubmit} class="space-y-6">
          ...
          <div class="flex gap-3 pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
            <Button variant="outline" type="button" onClick={props.onClose} disabled={props.isSaving} class="flex-1">
              {t("common.cancel")}
            </Button>
            <Button variant="primary" type="submit" loading={props.isSaving} class="flex-1">
